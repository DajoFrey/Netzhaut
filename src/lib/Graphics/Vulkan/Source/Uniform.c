...
NH_BEGIN()
    ...
    switch (type)
    {
        case NH_VK_UNIFORM_TOP_BORDER    : memcpy(color_p, Node_p->Properties.Border.Color.top_p, sizeof(float) * 4); break;
        case NH_VK_UNIFORM_RIGHT_BORDER  : memcpy(color_p, Node_p->Properties.Border.Color.right_p, sizeof(float) * 4); break;
        case NH_VK_UNIFORM_BOTTOM_BORDER : memcpy(color_p, Node_p->Properties.Border.Color.bottom_p, sizeof(float) * 4); break;
        case NH_VK_UNIFORM_LEFT_BORDER   : memcpy(color_p, Node_p->Properties.Border.Color.left_p, sizeof(float) * 4); break;
        case NH_VK_UNIFORM_BACKGROUND    : memcpy(color_p, Node_p->Properties.Background.color_p, sizeof(float) * 4); break;
...
//NH_BEGIN()
//
//    Nh_HTML_ClipBox ContentBox = Nh_CSS_getContentPixelBox(Node_p);
...
//    NH_CHECK(Nh_appendToLinkedList(&Context_p->Vulkan.Uniforms, Buffer_p))
//
//    NH_CHECK(Nh_Vk_createUniform(
...
NH_BEGIN()
    ...
    for (int i = 0; i < Node_p->Vulkan.Uniforms.count; ++i) {
        NH_Vk_Buffer *Buffer_p = Nh_getFromLinkedList(&Node_p->Vulkan.Uniforms, i);
        Nh_Vk_destroyBuffer(Driver_p, Buffer_p);
    }

    Nh_destroyLinkedList(&Node_p->Vulkan.Uniforms, true);
...
{
NH_BEGIN()
NH_END(Nh_getFromLinkedList(&Node_p->Vulkan.Uniforms, index))
}
...
NH_BEGIN()
    ...
    *Data_pp = Nh_allocate(sizeof(NH_Vk_Buffer));
    ...
    Nh_appendToLinkedList(&Node_p->Vulkan.Uniforms, *Data_pp);
