...
static NH_RESULT Nh_Vk_createTextVertices(
    Nh_Vk_Host *Host_p, Nh_Vk_Driver *Driver_p, Nh_HTML_Node *Node_p, float *vertices_p, uint32_t *indices_p
);
...
NH_BEGIN()
    ...
    Nh_Texture *Texture_p = Nh_Gfx_getTexture(Content_p, &Node_p->Properties.Text.Font_p->URI);
    ...
    NH_CHECK(Nh_Vk_createTextVertices(
        Nh_Vk_getHost(), &GPU_p->Driver, Node_p, vertices_p, indices_p
    ))
...
NH_BEGIN()
    ...
    VkBufferCreateInfo bufferCreateInfo = 
    {
        ...
        .size        = strlen(Node_p->text_p) * 20 * sizeof(float),
    ...
    bufferCreateInfo.size  = strlen(Node_p->text_p) * 6 * sizeof(uint32_t);
...
NH_BEGIN()
    ...
    // text color, etc.
    float fsData_p[11];
    fsData_p[0] = Node_p->Properties.Text.color_p[0];
    fsData_p[1] = Node_p->Properties.Text.color_p[1];
    fsData_p[2] = Node_p->Properties.Text.color_p[2];
    fsData_p[3] = Node_p->Properties.Text.color_p[3];
    fsData_p[10] = 0.04f; // TODO (float)Node_p->Properties.Text.fontSize;
...
NH_BEGIN()
    ...
    Nh_appendToLinkedList(&Node_p->Vulkan.Buffers, Buffer_p);
    ...
    Nh_appendToLinkedList(&Node_p->Vulkan.Uniforms, Buffer_p);
    ...
    Nh_appendToLinkedList(&Node_p->Vulkan.Descriptors, Descriptor_p);
