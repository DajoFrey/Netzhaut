...
//    NH_CHECK(Nh_appendToLinkedList(&Context_p->Vulkan.Descriptors, Descriptor_p))
//
//    VkDescriptorSetLayoutBinding setLayoutBindings[2];
//    VkDescriptorSetLayoutBinding setLayoutBinding1 =
//    {
...
NH_BEGIN()
    ...
    for (int i = 0; i < Node_p->Vulkan.Descriptors.count; ++i) {
        NH_Vk_DescriptorSet *Descriptor_p = Nh_getFromLinkedList(&Node_p->Vulkan.Descriptors, i);
        Nh_Vk_destroyDescriptorSet(Driver_p, Descriptor_p, &(Driver_p->DescriptorPool_p[0])); // TODO
    }

    Nh_destroyLinkedList(&Node_p->Vulkan.Descriptors, true);
...
{
NH_BEGIN()
NH_END(Nh_getFromLinkedList(&Node_p->Vulkan.Descriptors, index))
}
...
NH_BEGIN()
    ...
    *Data_pp = Nh_allocate(sizeof(NH_Vk_DescriptorSet));
    ...
    Nh_appendToLinkedList(&Node_p->Vulkan.Descriptors, *Data_pp);
