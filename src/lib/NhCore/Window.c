...
static NH_RESULT Nh_processConfigureEvent(
    Nh_Window *Window_p, Nh_IO_ConfigureEvent *Event_p)
{
NH_BEGIN()
...
static NH_RESULT Nh_processMouseEvent(
    Nh_Window *Window_p, Nh_IO_MouseEvent *Event_p)
{
NH_BEGIN()
    ...
    if (Hit_p->Settings.priority != 0 && Event_p->trigger == NH_IO_TRIGGER_PRESS) {Nh_focusTab(Hit_p);} 

    Nh_IO_MouseEvent *New_p = Nh_advanceRingIterator(&Hit_p->Input.Mouse.Events);
...
NH_BEGIN()
    ...
    while (1) {
        Nh_IO_ConfigureEvent *Event_p = Nh_incrementRingIteratorMark(
...
//    NH_CHECK(NULL, ((Nh_IO_createWindow_f)Nh_loadFunction("NhIO", "Nh_IO_createWindow"))(&Window_p->IO, "Empty Window", Window_p->Size))
...
//            Nh_IO_setWindowBackgroundColor(&Window_p->IO, *((Nh_Color*)Window_p->args_p));
...
//            }
//
//            Nh_IO_setWindowTitle(&Window_p->IO, Window_p->args_p);
//            Window_p->signal = NH_SIGNAL_DONE;
//       
//            break;
        }
...
//    }
//            NH_CHECK(NH_SIGNAL_ERROR, Nh_IO_destroyWindow(&Window_p->IO))
//
//            Nh_free(Window_p->Settings.title_p);
//            Nh_free(Window_p);
//
