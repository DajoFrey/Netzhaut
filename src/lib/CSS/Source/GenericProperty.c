...
NH_BEGIN()
    ...
    for (int i = 0; i < Node_p->Properties.Candidates.count; ++i) {
        Nh_CSS_configureGenericProperty(
            Node_p, Nh_CSS_getProperty(&Node_p->Properties.Candidates, i)
        );
    }
...
void Nh_CSS_getGenericProperties(
    Nh_HTML_Node *Node_p, Nh_CSS_GenericProperty *Properties_pp[NH_CSS_PROPERTY_COUNT])
{
NH_BEGIN()

    for (int i = 0; i < Node_p->Properties.Candidates.count; ++i) 
    {
        Nh_CSS_GenericProperty *Property_p = Nh_CSS_getProperty(&Node_p->Properties.Candidates, i);
...
NH_RESULT Nh_CSS_addGenericProperty(
    Nh_LinkedList *Properties_p, Nh_CSS_GenericProperty *Copy_p)
{
NH_BEGIN()

    NH_CHECK_NULL(Properties_p)
    NH_CHECK_NULL(Copy_p)
    ...
    Nh_appendToLinkedList(Properties_p, Property_p);

NH_DIAGNOSTIC_END(NH_SUCCESS)
}

Nh_CSS_GenericProperty *Nh_CSS_getProperty(
    Nh_LinkedList *Properties_p, int index)
{
NH_BEGIN()
NH_END(Nh_getFromLinkedList(Properties_p, index))
}

void Nh_CSS_destroyGenericProperties(
    Nh_LinkedList *Properties_p)
{
NH_BEGIN()
    ...
    {
        Nh_CSS_GenericProperty *Property_p = Nh_getFromLinkedList(Properties_p, i);
        ...
        if (Property_p->Selector.Parts.count > 0) {
            if (destroyed_p != Property_p->Selector.Parts.Head_p) {
                ...
                for (int j = 0; j < Property_p->Selector.Parts.count; ++j) {
                    Nh_CSS_Selector *Part_p = Nh_getFromLinkedList(&Property_p->Selector.Parts, j);
                    Nh_free(Part_p->string_p);
                }
                Nh_destroyLinkedList(&Property_p->Selector.Parts, true);
            }
        }
    }

    Nh_destroyLinkedList(Properties_p, true);
