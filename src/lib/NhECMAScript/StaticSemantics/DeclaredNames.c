    ...
    switch (Node_p->type)
    {
        case NH_ECMASCRIPT_PARSE_NODE_BLOCK :
            if (Node_p->Children.size == 3) {
                NH_ECMASCRIPT_END(Nh_ECMAScript_getLexicallyDeclaredNames(Node_p->Children.handles_pp[1]))
            }
            break;

        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST :
        {
            Nh_List Names = Nh_ECMAScript_getLexicallyDeclaredNames(Node_p->Children.handles_pp[0]);
            if (Node_p->Children.size == 2) {
                Nh_List StatementNames = Nh_ECMAScript_getLexicallyDeclaredNames(Node_p->Children.handles_pp[1]);
        ...
        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST_ITEM :
            if (((Nh_ECMAScript_ParseNode*)Node_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_STATEMENT) {
                ...
                if (((Nh_ECMAScript_ParseNode*)Statement_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_LABELLED_STATEMENT) {
                    NH_ECMASCRIPT_END(Nh_ECMAScript_getLexicallyDeclaredNames(Statement_p->Children.handles_pp[0]))
                }
            }
            else { // must be Declaration
                Nh_ECMAScript_ParseNode *Declaration_p = Node_p->Children.handles_pp[0];
                NH_ECMASCRIPT_END(Nh_ECMAScript_getBoundNames(Declaration_p->Children.handles_pp[0]))
            }
            break;

        case NH_ECMASCRIPT_PARSE_NODE_SCRIPT_BODY :
        {
            Nh_ECMAScript_ParseNode *StatementList_p = Node_p->Children.handles_pp[0];
        ...
        case NH_ECMASCRIPT_PARSE_NODE_MODULE_ITEM_LIST :
        {
            Nh_List Names = Nh_ECMAScript_getLexicallyDeclaredNames(Node_p->Children.handles_pp[0]);
            if (Node_p->Children.size == 2) {
                Nh_List ModuleNames = Nh_ECMAScript_getLexicallyDeclaredNames(Node_p->Children.handles_pp[1]);
        ...
        case NH_ECMASCRIPT_PARSE_NODE_MODULE_ITEM :
            if (((Nh_ECMAScript_ParseNode*)Node_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_IMPORT_DECLARATION) {
                ...
                if (((Nh_ECMAScript_ParseNode*)ExportDeclaration_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_VARIABLE_STATEMENT) {
                    break;
                } 
                NH_ECMASCRIPT_END(Nh_ECMAScript_getBoundNames(ExportDeclaration_p))
            }
            else if (((Nh_ECMAScript_ParseNode*)Node_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST_ITEM) {
                NH_ECMASCRIPT_END(Nh_ECMAScript_getLexicallyDeclaredNames(Node_p->Children.handles_pp[0]))
            }          
            break;
...
NH_ECMASCRIPT_BEGIN()
    ...
    switch (Node_p->type)
    {
        case NH_ECMASCRIPT_PARSE_NODE_BLOCK :
            if (Node_p->Children.size == 3) {
                NH_ECMASCRIPT_END(Nh_ECMAScript_getTopLevelLexicallyDeclaredNames(Node_p->Children.handles_pp[1]))
            }
            break;

        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST :
        {
            Nh_List Names = Nh_ECMAScript_getTopLevelLexicallyDeclaredNames(Node_p->Children.handles_pp[0]);
            if (Node_p->Children.size == 2) {
                Nh_List StatementNames = Nh_ECMAScript_getTopLevelLexicallyDeclaredNames(Node_p->Children.handles_pp[1]);
                Nh_appendItemsToList(&Names, &StatementNames);
                Nh_freeList(&StatementNames, NH_FALSE);
            }
            NH_ECMASCRIPT_END(Names)
...
NH_ECMASCRIPT_BEGIN()
    ...
    switch (Node_p->type)
    {
        ...
        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT :
            ...
            NH_ECMASCRIPT_END(Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[0]))

        case NH_ECMASCRIPT_PARSE_NODE_BLOCK :
            if (Node_p->Children.size == 3) {
                NH_ECMASCRIPT_END(Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]))
            }
            break;

        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST :
            {
                Nh_List Names = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[0]);
                if (Node_p->Children.size == 2) {
                    Nh_List StatementNames = Nh_ECMAScript_getBoundNames(Node_p->Children.handles_pp[1]);
                    Nh_appendItemsToList(&Names, &StatementNames);
                    Nh_freeList(&StatementNames, NH_FALSE);
                }
                NH_ECMASCRIPT_END(Names)
...
//            if (Node_p->Children.size == 3)
//            {
//                Nh_List FirstStatementNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]);
//                Nh_List SecondStatementNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[2]);
//                for (int i = 0; i < SecondStatementNames.size; ++i) {
//                    Nh_appendToList(&FirstStatementNames, SecondStatementNames.handles_pp[i]);
//                }
...
//                Nh_List VariableDeclarationListNames = Nh_ECMAScript_getBoundNames(Node_p->Children.handles_pp[0]);
//                Nh_List StatementNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[Node_p->Children.size - 1]);
//                for (int i = 0; i < StatementNames.size; ++i) {
//                    Nh_appendToList(&VariableDeclarationListNames, StatementNames.handles_pp[i]);
//                }
//                Nh_freeList(&StatementNames, NH_FALSE);
//                NH_ECMASCRIPT_END(VariableDeclarationListNames)
//            }
//            else {
//                NH_ECMASCRIPT_END(Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[Node_p->Children.size - 1]))
//            }
//
...
//                Nh_List ForBindingNames = Nh_ECMAScript_getBoundNames(Node_p->Children.handles_pp[0]);
//                Nh_List StatementNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[Node_p->Children.size - 1]);
//                for (int i = 0; i < StatementNames.size; ++i) {
//                    Nh_appendToList(&ForBindingNames, StatementNames.handles_pp[i]);
//                }
//                Nh_freeList(&StatementNames, NH_FALSE);
...
//                    Names = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[0]);
//                    Nh_List DefaultClauseNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]);
//                    for (int i = 0; i < DefaultClauseNames.size; ++i) {
//                        Nh_appendToList(&Names, DefaultClauseNames.handles_pp[i]);
//                    }
//                    Nh_freeList(&DefaultClauseNames, NH_FALSE);
//                    if (Node_p->Children.size > 2) {
//                        Nh_List CaseClausesNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]);
//                        for (int i = 0; i < CaseClausesNames.size; ++i) {
//                            Nh_appendToList(&Names, CaseClausesNames.handles_pp[i]);
//                        }
...
//                Nh_List Names = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[0]);
//                if (Node_p->Children.size == 2) {
//                    Nh_List ClauseNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]);
//                    for (int i = 0; i < ClauseNames.size; ++i) {
//                        Nh_appendToList(&Names, ClauseNames.handles_pp[i]);
//                    }
...
//            if (Node_p->Children.size == 2) {
//                Nh_List BlockNames = Nh_ECMAScript_getBoundNames(Node_p->Children.handles_pp[0]);
//                Nh_List CatchOrFinallyNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]);
//                for (int i = 0; i < CatchOrFinallyNames.size; ++i) {
//                    Nh_appendToList(&BlockNames, CatchOrFinallyNames.handles_pp[i]);
//                }
...
//                Nh_List BlockNames = Nh_ECMAScript_getBoundNames(Node_p->Children.handles_pp[0]);
//                Nh_List CatchNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[1]);
//                Nh_List FinallyNames = Nh_ECMAScript_getVarDeclaredNames(Node_p->Children.handles_pp[2]);
//                for (int i = 0; i < CatchNames.size; ++i) {
//                    Nh_appendToList(&BlockNames, CatchNames.handles_pp[i]);
//                }
//                for (int i = 0; i < FinallyNames.size; ++i) {
//                    Nh_appendToList(&BlockNames, FinallyNames.handles_pp[i]);
//                }
...
//                NH_ECMASCRIPT_END(Nh_ECMAScript_getTopLevelVarDeclaredNames(Node_p->Children.handles_pp[0]))
//            }
//            break;
//
...
NH_ECMASCRIPT_BEGIN()
    ...
    switch (Node_p->type)
    {
        case NH_ECMASCRIPT_PARSE_NODE_BLOCK :
            if (Node_p->Children.size == 3) {
                NH_ECMASCRIPT_END(Nh_ECMAScript_getTopLevelVarDeclaredNames(Node_p->Children.handles_pp[1]))
            }
            break;

        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST :
            {
                Nh_List Names = Nh_ECMAScript_getTopLevelVarDeclaredNames(Node_p->Children.handles_pp[0]);
                if (Node_p->Children.size == 2) {
                    Nh_List StatementNames = Nh_ECMAScript_getTopLevelVarDeclaredNames(Node_p->Children.handles_pp[1]);
                    Nh_appendItemsToList(&Names, &StatementNames);
                    Nh_freeList(&StatementNames, NH_FALSE);
                }
                NH_ECMASCRIPT_END(Names)
        ...
        case NH_ECMASCRIPT_PARSE_NODE_STATEMENT_LIST_ITEM :
            if (((Nh_ECMAScript_ParseNode*)Node_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_DECLARATION) 
            {
                Nh_ECMAScript_ParseNode *Declaration_p = Node_p->Children.handles_pp[0];
                if (((Nh_ECMAScript_ParseNode*)Declaration_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_HOISTABLE_DECLARATION) {
                    NH_ECMASCRIPT_END(Nh_ECMAScript_getBoundNames(((Nh_ECMAScript_ParseNode*)Declaration_p->Children.handles_pp[0])->Children.handles_pp[0]))
                }
            }
            else {
                Nh_ECMAScript_ParseNode *Statement_p = Node_p->Children.handles_pp[0];
                if (((Nh_ECMAScript_ParseNode*)Statement_p->Children.handles_pp[0])->type == NH_ECMASCRIPT_PARSE_NODE_LABELLED_STATEMENT) {
                    NH_ECMASCRIPT_END(Nh_ECMAScript_getTopLevelVarDeclaredNames(Statement_p->Children.handles_pp[0]))
                }
        ...
        case NH_ECMASCRIPT_PARSE_NODE_LABELLED_STATEMENT :
            NH_ECMASCRIPT_END(Nh_ECMAScript_getTopLevelVarDeclaredNames(Node_p->Children.handles_pp[2]))
            break;
