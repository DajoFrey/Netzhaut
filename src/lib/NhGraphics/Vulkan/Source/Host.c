...
NH_BEGIN();
    ...
    if (Nh_Vk_validationLayerSupported(Host_p) == NH_FALSE ||  Nh_Vk_validationExtensionSupported(Host_p) == NH_FALSE) {
        NH_DIAGNOSTIC_END(NH_VULKAN_ERROR_VALIDATION_NOT_SUPPORTED)
    }
    ...
    if (function == NULL) {NH_DIAGNOSTIC_END(NH_VULKAN_ERROR_LOADER_CANT_CREATE_DEBUG_MESSENGER)}
...
NH_BEGIN();
    ...
    if (availableLayers_p == NULL) {NH_DIAGNOSTIC_END(NH_FALSE)}
...
NH_BEGIN()
    ...
    if (extensionProps_p == NULL) {NH_DIAGNOSTIC_END(NH_FALSE)}
