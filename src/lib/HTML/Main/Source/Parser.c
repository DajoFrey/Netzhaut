...
static NH_RESULT Nh_HTML_parseElement(
    char *current_p, Nh_HTML_Node *Node_p, Nh_HTML_Node *Parent_p, char **end_pp
);
...
static NH_RESULT Nh_HTML_allocateAttributes(
    Nh_LinkedList *Attributes_p, Nh_String **Name_pp, Nh_String **Value_pp
);
...
NH_BEGIN()
    ...
    char *data_p = Nh_getPayload(&Document_p->Content_p->URI);
    if (data_p == NULL) {NH_DIAGNOSTIC_END(NH_ERROR_BAD_STATE)}
    ...
    if (begin_p == NULL) {NH_DIAGNOSTIC_END(NH_HTML_ERROR_DOCUMENT_EMPTY)}

    char *end_p = NULL;

    Document_p->Tree.Root_p = Nh_allocate(sizeof(Nh_HTML_Node));
    NH_CHECK_MEM(Document_p->Tree.Root_p)
    NH_CHECK(Nh_HTML_parseElement(begin_p, Document_p->Tree.Root_p, NULL, &end_p))

    Nh_freePayload(&Document_p->Content_p->URI);
...
NH_BEGIN()
    ...
    char replace;
    *end_pp = Nh_HTML_seekEndOfElement(current_p, Parent_p == NULL ? -1 : Parent_p->tag);
    ...
    **end_pp = '\0';
    ...
    if (!IS_VOID_TAG(tag)) {NH_CHECK(Nh_HTML_parseContent(current_p, Node_p))}
...
NH_BEGIN()
    ...
    for (int i = 1; current_p[i] != ' '; current_p = &current_p[i]) 
    {
        ...
        if (current_p[i] == '>' && !inText) {NH_DIAGNOSTIC_END(NH_SUCCESS)}
    }

    current_p = &current_p[1];
    inText = false;

    Nh_LinkedList Attributes;
    NH_INIT_LINKED_LIST(Attributes)
...
NH_BEGIN()
    ...
    NH_BOOL inAttributeValue = NH_FALSE;
    int startTagLength;
    ...
    while (current_p != NULL)
    {
        ...
        for (int i = 0; current_p != NULL && (*current_p != '<' || current_p[1] == ' ' || current_p[1] == '\n') && *current_p != '\0'; current_p = current_p + 1) 
        {
            ...
            if (whiteSpaceCount <= 1 && *current_p != '\r') {
                NH_CHECK(Nh_appendCharToStringConstructor(String_p, *current_p))
            }
        }

        char *chars_p = Nh_getChars2(String_p);
        Nh_clearStringConstructor(String_p);        

        if (chars_p != NULL && strlen(chars_p) > 0) 
        {
            Nh_HTML_Node *Text_p = Nh_allocate(sizeof(Nh_HTML_Node));
            NH_CHECK_MEM(Text_p)
            Nh_HTML_initNode(Text_p, Node_p, NH_HTML_TAG_TEXT);
            NH_CHECK(Nh_setListData(&Node_p->Children, Text_p, -1))
            Text_p->text_p = chars_p;

            for (int i = 0; i < strlen(chars_p) && !Nh_HTML_isMetaNode(Node_p); ++i) 
            {
                if (chars_p[i] == '\n') {continue;}
                Nh_HTML_Node *Char_p = Nh_allocate(sizeof(Nh_HTML_Node));
                NH_CHECK_MEM(Char_p)
                Nh_HTML_initNode(Char_p, Text_p, NH_HTML_TAG_CHAR);
                NH_CHECK(Nh_setListData(&Text_p->Children, Char_p, -1))
                Char_p->text_p = &chars_p[i];
            }
        }
        ...
        if (tag != -1 && current_p[0] == '<' && current_p[1] != '/' && current_p[1] != ' ') 
        {
            ...
            char *end_p = NULL;
            NH_CHECK(Nh_HTML_parseElement(current_p, Child_p, Node_p, &end_p))
            NH_CHECK(Nh_setListData(&Node_p->Children, Child_p, -1))
            current_p = end_p;
        } 
        else {break;}
    }
...
NH_BEGIN()
    ...
    *Name_pp  = Nh_allocateString(NULL);
    NH_CHECK_MEM(*Name_pp) 

    *Value_pp = Nh_allocateString(NULL);
    NH_CHECK_MEM(*Value_pp)
