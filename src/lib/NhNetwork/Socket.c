...
NH_NET_RESULT Nh_Net_getSockets(
    char *hostNameIn_p, char *hostNameOut_p, NH_NET_PORT port, Nh_LinkedList *List_p)
{
NH_NET_BEGIN()
    ...
    if (s != 0) {NH_NET_DIAGNOSTIC_END(NH_NET_ERROR_BAD_STATE)}
    ...
    for (int i = 0; Info_p != NULL; Info_p = Info_p->ai_next, ++i)
    {
        ...
        *Socket_p = Nh_Net_initSocket();
        ...
        Socket_p->protocolName_p = Nh_allocateBytes(Proto_p->p_name);
        endprotoent();
        ...
        switch (Socket_p->family) 
        {
            case NH_NET_PROTOCOL_FAMILY_IPV4 :
            {
                ...
                if (getnameinfo((struct sockaddr *) Addr_p, sizeof(struct sockaddr_in), hbuf, sizeof(hbuf), NULL, 0, NI_NAMEREQD) == 0) {
                    Socket_p->hostName_p = Nh_allocateBytes(hbuf);
                }
        ...
        NH_CHECK(NH_NET_ERROR_BAD_STATE, Nh_appendToLinkedList(List_p, Socket_p))
    } 
...
NH_NET_BEGIN()
    ...
    {
        Nh_Net_ClientSocket *Tmp_p = Nh_getFromLinkedList(&Nh_Net_getNetwork()->Sockets, i);
...
#include NH_NET_CUSTOM_CHECK
    ...
    NH_NET_CHECK_NULL(NULL, Socket_p)
...
NH_NET_RESULT Nh_Net_closeClientSocket(
    Nh_Net_ClientSocket *Socket_p)
{
NH_NET_BEGIN()

//    Socket_p->count--;
//    if (Socket_p->count > 0) {NH_NET_DIAGNOSTIC_END(NH_NET_SUCCESS)} // socket is still in use
//
...
//    if (result != 0) {NH_NET_DIAGNOSTIC_END(NH_NET_ERROR_BAD_STATE)}
//
//#endif
//
//    Nh_removeFromLinkedList2(&Nh_Net_getNetwork()->Sockets, Socket_p, true);

NH_NET_DIAGNOSTIC_END(NH_NET_SUCCESS)
}

NH_NET_RESULT Nh_Net_freeSockets(
    Nh_LinkedList *Sockets_p)
{
NH_NET_BEGIN()

    for (int i = 0; i < Sockets_p->count; ++i) {
        Nh_Net_Socket *Socket_p = Nh_getFromLinkedList(Sockets_p, i);
        Nh_free(Socket_p->protocolName_p);
        Nh_free(Socket_p->hostName_p);
    }

    Nh_destroyLinkedList(Sockets_p, true);
...
NH_NET_RESULT Nh_Net_connect(
    char *hostName_p, Nh_Net_ClientSocket *ClientSocket_p, Nh_Net_Socket Socket)
{
NH_NET_BEGIN()

    if (ClientSocket_p == NULL || ClientSocket_p->Socket.family != Socket.family || ClientSocket_p->Socket.type != Socket.type) {
        NH_NET_DIAGNOSTIC_END(NH_NET_ERROR_BAD_STATE)
    }
    ...
    switch (Socket.family)
    {
        ...
        default : NH_NET_DIAGNOSTIC_END(NH_NET_ERROR_BAD_STATE)
    }
    ...
    if (Socket.type == NH_NET_SOCKET_TCP && Socket.port == NH_NET_PORT_HTTPS) {
        ClientSocket_p->SSL_p = Nh_Net_connectViaSSL(hostName_p, ClientSocket_p->fd);
        if (ClientSocket_p->SSL_p == NULL) {NH_NET_DIAGNOSTIC_END(NH_NET_ERROR_BAD_STATE)}
    }
...
NH_NET_BEGIN()
    ...
    if (messageSize > 0) {Nh_Net_logSend(message_p);}
...
NH_NET_END(messageSize == result ? NH_NET_SUCCESS : NH_NET_ERROR_BAD_STATE)
}
...
#include NH_NET_CUSTOM_CHECK
    ...
    switch (port)
    {
        ...
        case NH_NET_PORT_HTTPS : NH_NET_CHECK(Data, Nh_Net_sendDataRequestViaHTTP(Socket_p, hostName_p, pathName_p, secure)) break; 
        default : NH_NET_END(Data)
    }

    Nh_String String = Nh_initString(512); 
    ...
    do {
        ...
        if (size > 0) {
            NH_CHECK(Data, Nh_appendFormatToString(&String, set_p))
            Data.size += size; 
        }
    }
    while (size > 0);
...
NH_NET_BEGIN()
    ...
    switch (Socket_p->family)
    {
        ...
        {
            for (int i = 0; i < 4; ++i) {
                if (i < 3) {Nh_appendFormatToString(&String, "%d.", (Socket_p->address4 >> (8*i)) & 0xff);}
                else {Nh_appendFormatToString(&String, "%d", (Socket_p->address4 >> (8*i)) & 0xff);}
            }
            break;
        }
        case NH_NET_PROTOCOL_FAMILY_IPV6 :
        {
            for (int i = 0; i < 16; ++i) {
                if ((i % 2) == 0 || i == 0 || i == 15) {Nh_appendFormatToString(&String, "%02x", (int)Socket_p->address16_p[i]);}
                else if ((i % 2) == 1 && i != 0) {Nh_appendFormatToString(&String, "%02x:", (int)Socket_p->address16_p[i]);}
            }
            break;
        }
    }
...
NH_NET_BEGIN()
    ...
    Nh_appendFormatToString(&String, "%s %s %s %d", Socket_p->hostName_p, Socket_p->protocolName_p, addr_p, Socket_p->port);
...
NH_NET_BEGIN()
    ...
    switch (type)
    {
        case AF_INET  : NH_NET_DIAGNOSTIC_END(NH_NET_PROTOCOL_FAMILY_IPV4)
        case AF_INET6 : NH_NET_DIAGNOSTIC_END(NH_NET_PROTOCOL_FAMILY_IPV6) 
    }
...
NH_NET_BEGIN()
    ...
    switch (type)
    {
        case NH_NET_PROTOCOL_FAMILY_IPV4 : NH_NET_END(AF_INET)
        case NH_NET_PROTOCOL_FAMILY_IPV6 : NH_NET_END(AF_INET6) 
    }
...
NH_NET_BEGIN()
    ...
    switch (type)
    {
        case SOCK_STREAM    : NH_NET_DIAGNOSTIC_END(NH_NET_SOCKET_TCP)
        case SOCK_DGRAM     : NH_NET_DIAGNOSTIC_END(NH_NET_SOCKET_UDP) 
        case SOCK_SEQPACKET : NH_NET_DIAGNOSTIC_END(NH_NET_SOCKET_SEQPACKET)
        case SOCK_RAW       : NH_NET_DIAGNOSTIC_END(NH_NET_SOCKET_RAW)
        case SOCK_RDM       : NH_NET_DIAGNOSTIC_END(NH_NET_SOCKET_RDM)
    }
...
NH_NET_BEGIN()
    ...
    switch (type)
    {
        case NH_NET_SOCKET_TCP       : NH_NET_END(SOCK_STREAM)
        case NH_NET_SOCKET_UDP       : NH_NET_END(SOCK_DGRAM) 
        case NH_NET_SOCKET_SEQPACKET : NH_NET_END(SOCK_SEQPACKET)
        case NH_NET_SOCKET_RAW       : NH_NET_END(SOCK_RAW)
        case NH_NET_SOCKET_RDM       : NH_NET_END(SOCK_RDM)
    }
