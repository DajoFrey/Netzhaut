...
DEFINE_TOKEN: ;
    ...
    Nh_WebIDL_Token *Token_p = Nh_getFromArray(Tokens_p, -1);
    Token_p->type = type;
...
NH_WEBIDL_BEGIN()
    ...
    for (int i = 0; i < DirtyTokens.length; ++i) 
    {
        ...
        Nh_WebIDL_Token *TokenCopy_p = Nh_getFromArray(&CleanTokens, -1);
...
NH_WEBIDL_BEGIN()
    ...
    Nh_WebIDL_logTokenizer(logName_p, &DirtyTokens, NH_TRUE);

    Nh_Array CleanTokens = Nh_WebIDL_discardRedundantTokens(DirtyTokens);
    Nh_WebIDL_logTokenizer(logName_p, &CleanTokens, NH_FALSE);
