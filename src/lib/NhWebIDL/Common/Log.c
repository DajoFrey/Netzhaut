...
NH_WEBIDL_BEGIN()
    ...
    for (int i = 0; i < Tokens_p->length; ++i) 
    {
        ...
        sprintf(
            message_p, dirty ? "[NhWebIDL:%s:TokensDirty]{%s [%s]}" : "[NhWebIDL:%s:TokensClean]{%s [%s]}",
...
NH_WEBIDL_BEGIN()
    ...
    if (ParseNode_p->Token_p == NULL) {
        sprintf(
            message_p, "[NhWebIDL:%s:ParseTree]{%s%s}", fragmentName_p, indent_p, 
            NH_WEBIDL_PARSE_NODE_NAMES_PP[ParseNode_p->type]
        );
    }
    else {
        sprintf(
            message_p, "[NhWebIDL:%s:ParseTree]{%sTERMINAL %s}", fragmentName_p, indent_p, 
