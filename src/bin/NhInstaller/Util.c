...
NH_INSTALLER_END(dl_p)
}

void *Nh_Installer_loadFunction(
    void *dl_p, const NH_BYTE *functionName_p)
{
NH_INSTALLER_BEGIN()
    ...
    void *function_p = dlsym(dl_p, functionName_p); 
    if ((error_p = dlerror()) != NULL) {NH_INSTALLER_END(NULL)}

NH_INSTALLER_END(function_p)
}
