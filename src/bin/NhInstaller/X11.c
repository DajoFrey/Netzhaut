...
static NH_INSTALLER_RESULT Nh_Installer_connectToDisplay()
{
NH_INSTALLER_BEGIN()

    Data.Display_p = XOpenDisplay(NULL);
    NH_INSTALLER_CHECK_NULL(Data.Display_p)
...
static NH_INSTALLER_RESULT Nh_Installer_createSimpleWindow(
...
static void Nh_Installer_createGraphicsContext()
{
...
static void Nh_Installer_createFont()
{
...
NH_INSTALLER_BEGIN()
    ...
    NH_INSTALLER_CHECK(Nh_Installer_connectToDisplay())
...
// create window
    Nh_Installer_createSimpleWindow(width, height);
    Nh_Installer_createGraphicsContext();
    Nh_Installer_createFont();
...
NH_INSTALLER_BEGIN()
    ...
    while (XPending(Data.Display_p) > 0) 
    {
        ...
        {   
                 if (Event.xclient.data.l[0] == Data.DeleteAtom) {Nh_Installer_handleGUIExit();}
        ...
        else if (Event.type == SelectionNotify)
        {
            ...
            NH_INSTALLER_CHECK(Nh_Installer_handleGUIExpose())
        }
    }
...
NH_INSTALLER_BEGIN()
    ...
    if (count != 0) {Nh_Installer_X11_drawTextLine(x, y + descent, text_p + count);}
